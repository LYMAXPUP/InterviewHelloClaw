...
class WorkspaceManager:
    ...
    def __init__(self, workspace_path: str):
        ...
        self.outputs_path = os.path.join(self.workspace_path, "outputs")
    ...
    def ensure_workspace_exists(self):
        ...
        os.makedirs(self.outputs_path, exist_ok=True)  # 临时输出文件目录
        ...
        # 复制默认 Skills（如果 skills 目录为空）
        self._copy_default_skills()
    ...
    def _copy_default_skills(self):
        """复制默认 Skills 到工作空间

        如果 skills 目录为空，从模板目录复制默认 Skills。
        """
        # 检查 skills 目录是否为空
        if os.path.exists(self.skills_path):
            existing_skills = [d for d in os.listdir(self.skills_path)
                             if os.path.isdir(os.path.join(self.skills_path, d))]
            if existing_skills:
                return  # 已有 skills，不覆盖
        ...
        # 复制每个 skill 目录（排除 .gitkeep）
        import shutil
        for entry in template_skills_dir.iterdir():
            if entry.is_dir():
                target_dir = os.path.join(self.skills_path, entry.name)
                if not os.path.exists(target_dir):
                    try:
                        shutil.copytree(entry, target_dir)
                        print(f"✅ 已复制默认 Skill: {entry.name}")
                    except Exception as e:
                        print(f"⚠️ 复制 Skill 失败: {entry.name} - {e}")
