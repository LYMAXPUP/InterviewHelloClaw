"""网页搜索工具 - 使用 Tavily Search API 进行网络搜索"""
...
class WebSearchTool(Tool):
    ...
    需要配置环境变量 TAVILY_KEY 或在初始化时传入 API key。
    """
    ...
    def __init__(
        ...
        Args:
            api_key: Tavily Search API key，如未提供则从环境变量 TAVILY_KEY 读取
    ...
    def _search(self, query: str, count: int = None) -> ToolResponse:
        ...
        if not self.api_key:
            return ToolResponse.error(
                ...
                message="未配置 Tavily API Key。请设置环境变量 TAVILY_KEY 或在初始化时传入 api_key 参数"
            )
        ...
        try:
            ...
            payload = {
                "query": query,
                "max_results": count or self.max_results,
                "search_depth": "basic",
            }
