...
# 白名单命令（只允许这些基础命令）
ALLOWED_COMMANDS = [
    "ls", "cd", "cat", "echo", "pwd", "git", "npm", "pnpm", "uv", "python",
    "python3", "node", "yarn", "pip", "pip3", "mkdir", "touch", "cp",
    "mv", "grep", "find", "head", "tail", "wc", "sort", "uniq", "date", "dir"
]
...
class ExecuteCommandTool(Tool):
    ...
    def __init__(
        ...
        Args:
            ...
            max_output_size: 最大输出大小（字符），默认 100000
            timeout: 命令执行超时时间（秒），默认 30
            allowed_directories: 允许的工作目录列表，None 表示不限制
            default_workdir: 默认工作目录，如果命令未指定 workdir 则使用此目录
        """
        super().__init__(
    ...
    def _execute_command(
        ...
        # 如果没有指定 workdir，使用默认工作目录
        if not workdir and self.default_workdir:
        ...
        # 执行命令
        try:
            # 使用 UTF-8 编码，errors='replace' 防止无法解码时崩溃
