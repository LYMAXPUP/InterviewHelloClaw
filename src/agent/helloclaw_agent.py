...
class HelloClawAgent:
    ...
    def __init__(
        ...
        self.config = Config(
            ...
            trace_enabled=False,
            skills_enabled=True,  # 启用 Skill 支持
    ...
    def _build_system_prompt(self) -> str:
        ...
        if skills_prompt:
            context_parts.append(skills_prompt)
    ...
    def _setup_tools(self) -> ToolRegistry:
        ...
        registry.register_tool(ExecuteCommandTool(
            ...
            default_workdir=self.workspace_path,  # 默认在 workspace 目录执行
        ))
        registry.register_tool(WebSearchTool())  # 网页搜索工具
        registry.register_tool(LoadSkillTool(self.skill_manager))  # Skill 加载工具（渐进式披露）
    ...
    def chat(self, message: str, session_id: str = None, images: list = None) -> str:
        """同步聊天

        Args:
            message: 用户消息
            session_id: 会话 ID
            images: 图片 URL 列表（多模态支持）
        """
    ...
    async def achat(self, message: str, session_id: str = None, images: list = None):
        """异步聊天（支持流式输出）
        ...
        if images:
            llm_kwargs["images"] = images
            print(f"[⏱️ {time.time():.3f}] 包含 {len(images)} 张图片")
